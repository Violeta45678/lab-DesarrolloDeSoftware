    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (accion == null || accion.equals("listar")) {
                ...
                out.println("<th class='py-2 border border-brown-500'>Direccion</th>");
                out.println("<th class='py-2 border border-brown-500'>Rol</th>");
                out.println("<th class='py-2 border border-brown-500'>Fecha de Registro</th>");
                out.println("</tr>");
                out.println("</thead>");
                out.println("<tbody>");

                for (Cliente cliente : listaClientes) {
                    out.println("<tr class='text-center border'>");
                    ...
                    out.println("<td class='py-2'>" + cliente.getUsuarioId() + "</td>");
                ...
                out.println("<input type='text' name='nombre' value='" + (cliente != null ? cliente.getNombre() : "") + "' required>");
                
                out.println("<label class='block text-brown-700 font-medium mb-2'>Apellido:</label>");
                out.println("<input type='text' name='apellido' value='" + (cliente != null ? cliente.getApellido() : "") + "' required>");
                
                out.println("<label class='block text-brown-700 font-medium mb-2'>Dirección:</label>");
                out.println("<input type='text' name='direccion' value='" + (cliente != null ? cliente.getDireccion() : "") + "' required>");
                
                out.println("<label class='block text-brown-700 font-medium mb-2'>Usuario ID:</label>");
                out.println("<input type='number' name='usuarioId' value='" + (cliente != null ? cliente.getUsuarioId() : "") + "' required>");
                ...
                if (resultado) {
                    response.sendRedirect("ctrlClientes?accion=listar");
                } else {
